...
export class SpotifyControls {
  ...
  constructor() {
    const buttonsInfo = [
      ...
      { id: "pause", text: "$(primitive-square)" },
      {
        id: "playPause",
        text: "$(triangle-right)",
        dynamicText: (isPlaying?: boolean) =>
          isPlaying ? "$(primitive-square)" : "$(triangle-right)",
      },
      ...
      { id: "unmuteVolume", text: "$(unmute)" },
      {
        id: "muteUnmuteVolume",
        text: "$(mute)",
        dynamicText: (isMuted?: boolean) => (isMuted ? "$(mute)" : "$(unmute)"),
      },
      ...
      { id: "volumeDown", text: "$(arrow-small-down)" },
      {
        id: "toggleRepeating",
        text: "$(sync)",
        dynamicColor: (isRepeating?: boolean) =>
          isRepeating ? "white" : "darkgrey",
      },
      {
        id: "toggleShuffling",
        text: "$(git-branch)",
        dynamicColor: (isShuffling?: boolean) =>
          isShuffling ? "white" : "darkgrey",
      },
      { id: "lyrics", text: "$(book)" },
    ...
    this._buttons = buttonsInfo.map((item) => {
      ...
      const { title } = commands.filter(
        (command) => command.command === buttonCommand
      )[0] || { title: "" };
  ...
  updateDynamicButtons(
    ...
    changed =
      this._updateColor(this._toggleRepeatingButton, repeating) || changed;
    changed =
      this._updateColor(this._toggleShufflingButton, shuffling) || changed;
    return changed;
  }
  ...
  /**
   * Show buttons that arxe visible
   */
  showVisible() {
    this.buttons.forEach(
      (button) => button.visible && button.statusBarItem.show()
    );
  }
  showHtmlButtons() {
    this.buttons.forEach(
      (button) =>
        `<button name=${button.buttonName} onClick={${button.buttonCommand}} id=${button.id}>${button.text}</button>`
    );
  }
