...
export class SpotifyStatus {
  ...
  render() {
    ...
    if (state.isRunning) {
      ...
      if (text !== this._statusBarItem.text) {
        // we need this guard to prevent flickering
        this._statusBarItem.text = text;
        toRedraw = true;
      }
      if (
        this._spotifyControls.updateDynamicButtons(
          playing === "playing",
          volume === 0,
          isRepeating,
          isShuffling
  ...
  private formattedTrackInfo(track: ITrack): string {
    ...
    return getTrackInfoFormat().replace(
      /albumName|artistName|trackName/gi,
      (matched) => keywordsMap[matched]
    );
  }
}
